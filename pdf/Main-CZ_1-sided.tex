...
Poslední teoretickou kapitolou bude vsuvka do toerie grafů.
...
\begin{equation}
 p(y\vert x) = \mathcal{N}\left(\mathbb{X}\theta, \sigma^2 I \right)
 \end{equation}
...
\begin{equation}
q\left(z\vert x \right) = \mathcal{N}\left(\mu_{\phi}(x), \sigma^2_{\phi}(x) \right)
\end{equation}
...
\begin{equation}
z = \mu(x) + \sigma^2(x)\odot\epsilon 
\end{equation}
a druhý je analytické řešení KL-diveregence dvou gaussovských distribucí
\begin{equation}
\begin{split}
 D_{KL}\left(q\left(z\vert x \right) \Vert p(z)\right) & = \frac{1}{2}\left[\tr(\sigma^2(x)) - \mu\tran(x)\mu(x) - k - \log\det \left(\sigma^2(x)   \right)\right] \\
 & = \frac{1}{2}\left[\sum_{l = 1}^k(\sigma^2(x)) -\mu\tran(x)\mu(x) - k - \sum_{l = 1}^k\log\sigma^2(x)    \right]
\end{split}
\end{equation}
...
\begin{equation}
\hat{\theta},\hat{\phi} = \argmin\left[\sum_{i = 1}^n\sum_{j = 1}^p \left[ x_i - f_\theta \left(\mu(x_i) + \sigma^2(x_i)\cdot\epsilon_{i,j}  \right)        \right] - \frac{1}{2}\left[\sum_{l = 1}^k(\sigma^2_{\phi}(x_i)) -\mu_{\phi}\tran(x_i)\mu_{\phi}(x_i) - k - \sum_{l = 1}^k\log\sigma^2_{\phi}(x_i)   \right] \right]
 \end{equation}
\subsection*{Příklad}
...
\subsection*{Příklad}
Předpokládejme takový model který ke každému $y_i$, $i \in \hat{n}$ přiřazuje určitý počet $x_{j}^{(k)}$, přičemž celkový počet $x_{j}$ je $j \in \hat{m}$, $m>n$. Tento počet může být generován například Poissonovým rozdělením s danou střední hodnotou $\lambda$, navíc celkový součet těchto počtů musí být $m$.  Jednotlivá $y_i$ potom můžou být určena nějakou kvadratickou závislostí na průměrech k nim přiřazených $x_{j}^{k}$. Všechna $x_j$ jsou přitom generována pomocí uniformního rozdělení. Pokusme se nalézt sdruženou distribuci $p(y, \bar{x})$, kde $\bar{x}_j$ značí výběrový průměr $x_j$ . Použijeme opět součinové pravidlo 
\begin{equation}
p(y,\bar{x}_j) = p(y\vert \bar{x}_j)p(\bar{x}_j)
\end{equation}
a pokusme se nalézt tyto dvě distribuce. Tento postup je nám známý už z kapitoly \eqref{generative}. Pro určení podmíněné distribuce $p\left(y \vert \bar{x}_j\right)$ použijeme opět metodu nejmenších čtverců a dostaneme 
\begin{equation}
p\left(y \vert \bar{x}\right) = \mathcal{N}\left(\mathbb{X}\theta, \sigma^2 I \right)
\end{equation}
...
\begin{equation}
p(\bar{x}_j) = \mathcal{N}\left(\frac{1}{j}\sum_{j = 1}^m\bar{x}_{j}, std(\bar{x}_j) \right)
\end{equation}
